-- ============================================
-- QUERIES PARA ARREGLAR TABLAS MANUALMENTE
-- ============================================

-- OPCIÓN 1: Arreglar solo la tabla problemática
-- Ejecuta esto ANTES de importar el SQL completo

USE railway;

-- Eliminar la tabla problemática si existe
DROP TABLE IF EXISTS `inventario_plancanjeconfig`;

-- Crear la tabla SIN el DEFAULT (que causa el error)
...
-- ============================================
-- OPCIÓN 2: Crear solo las tablas críticas que faltan
-- ============================================

-- Tablas de inventario (crear primero las que no tienen dependencias)
...
-- Tablas de ventas (crear primero cupon, luego venta)
...
-- Agregar foreign keys después (solo si las tablas referenciadas existen)
-- Nota: Estas foreign keys pueden fallar si las tablas no existen, pero eso está OK
-- porque son opcionales (pueden ser NULL)
...
-- ============================================
-- OPCIÓN 3: Saltarse la tabla problemática al importar
-- ============================================
-- Si quieres importar el SQL completo pero saltarte esa tabla:
-- 1. Abre sistema_negocio_limpio.sql en un editor
-- 2. Busca la línea 1648 (CREATE TABLE inventario_plancanjeconfig)
-- 3. Comenta o elimina esa sección completa hasta encontrar la siguiente tabla
-- 4. O ejecuta primero el query de OPCIÓN 1 arriba para crear la tabla correctamente
-- 5. Luego importa el resto del SQL
